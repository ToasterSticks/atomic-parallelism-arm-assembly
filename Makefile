PROG = p3
CFLAGS = -Werror -Wall -O3 -g -std=c23
...
TEST_INS=${addsuffix .in,${TEST_NAMES}}
...
${TEST_OUTS} : %.out : Makefile %.run %.in
	@echo "failed to run" > $*.out
	@rm -f $*.err
	(/usr/bin/time --quiet -o $*.time -f "%E" timeout 10 ./$*.run < $*.in  > $*.out 2> $*.err); if [ $$? -eq 124 ]; then echo "timeout" > $*.time; fi
...
clean:
	-rm -rf ${PROG} *.out *.diff *.result *.d *.o *.time *.err *.s *.run
